...
def get_feynman_dataset(name):
    ...
    if name == 'special function1' or name == 121:
        symbol = x, y = symbols('x, y')
        expr = (x+y) * sin(exp(2*y)) #예시
        f = lambda x: torch.exp(torch.special.bessel_j0(20*x[:,[0]]) + x[:,[1]]**2)
        ranges = [-1,1]
        
    if name == 'Jacobian elliptic functions' or name == 121:
        u, m = symbols('u m')
        symbol = (u, m)
        expr   = 'ellipj(u, m)'
        f      = lambda X: special.ellipj(X[:,0], X[:,1])
        ranges = [[0, 2*tpi], [0, 1]]
        return symbol, expr, f, ranges

    # 122: Incomplete elliptic integral of the first kind
    if name == 'Incomplete elliptic integral of the first kind' or name == 122:
        φ, m = symbols('φ m')
        symbol = (φ, m)
        expr   = 'ellipkinc(φ, m)'
        f      = lambda X: special.ellipkinc(X[:,0], X[:,1])
        ranges = [[0, tpi], [0, 1]]
        return symbol, expr, f, ranges

    # 123: Incomplete elliptic integral of the second kind
    if name == 'Incomplete elliptic integral of the second kind' or name == 123:
        φ, m = symbols('φ m')
        symbol = (φ, m)
        expr   = 'ellipeinc(φ, m)'
        f      = lambda X: special.ellipeinc(X[:,0], X[:,1])
        ranges = [[0, tpi], [0, 1]]
        return symbol, expr, f, ranges

    # 124: Bessel function of the first kind
    if name == 'Bessel function of the first kind' or name == 124:
        ν, x = symbols('ν x')
        symbol = (ν, x)
        expr   = 'jv(ν, x)'
        f      = lambda X: special.jv(X[:,0], X[:,1])
        ranges = [[0, 10], [0, 20]]
        return symbol, expr, f, ranges

    # 125: Bessel function of the second kind
    if name == 'Bessel function of the second kind' or name == 125:
        ν, x = symbols('ν x')
        symbol = (ν, x)
        expr   = 'yv(ν, x)'
        f      = lambda X: special.yv(X[:,0], X[:,1])
        ranges = [[0, 10], [0.1, 20]]
        return symbol, expr, f, ranges

    # 126: Modified Bessel function of the second kind
    if name == 'Modified Bessel function of the second kind' or name == 126:
        ν, x = symbols('ν x')
        symbol = (ν, x)
        expr   = 'kv(ν, x)'
        f      = lambda X: special.kv(X[:,0], X[:,1])
        ranges = [[0, 10], [0.1, 20]]
        return symbol, expr, f, ranges

    # 127: Modified Bessel function of the first kind
    if name == 'Modified Bessel function of the first kind' or name == 127:
        ν, x = symbols('ν x')
        symbol = (ν, x)
        expr   = 'iv(ν, x)'
        f      = lambda X: special.iv(X[:,0], X[:,1])
        ranges = [[0, 10], [0, 20]]
        return symbol, expr, f, ranges

    # 128: Associated Legendre function (m = 0)
    if name == 'Associated Legendre function (m = 0)' or name == 128:
        n, z = symbols('n z')
        symbol = (n, z)
        expr   = 'lpmv(0, n, z)'
        f      = lambda X: special.lpmv(0, X[:,0], X[:,1])
        ranges = [[0, 10], [-1, 1]]
        return symbol, expr, f, ranges

    # 129: Associated Legendre function (m = 1)
    if name == 'Associated Legendre function (m = 1)' or name == 129:
        n, z = symbols('n z')
        symbol = (n, z)
        expr   = 'lpmv(1, n, z)'
        f      = lambda X: special.lpmv(1, X[:,0], X[:,1])
        ranges = [[0, 10], [-1, 1]]
        return symbol, expr, f, ranges

    # 130: Associated Legendre function (m = 2)
    if name == 'Associated Legendre function (m = 2)' or name == 130:
        n, z = symbols('n z')
        symbol = (n, z)
        expr   = 'lpmv(2, n, z)'
        f      = lambda X: special.lpmv(2, X[:,0], X[:,1])
        ranges = [[0, 10], [-1, 1]]
        return symbol, expr, f, ranges

    # 131: spherical harmonics (m = 0, n = 1)
    if name == 'spherical harmonics (m = 0, n = 1)' or name == 131:
        φ, θ = symbols('φ θ')
        symbol = (φ, θ)
        expr   = 'sph_harm(0, 1, φ, θ)'
        f      = lambda X: special.sph_harm(0, 1, X[:,0], X[:,1])
        ranges = [[0, 2*tpi], [0, tpi]]
        return symbol, expr, f, ranges

    # 132: spherical harmonics (m = 1, n = 1)
    if name == 'spherical harmonics (m = 1, n = 1)' or name == 132:
        φ, θ = symbols('φ θ')
        symbol = (φ, θ)
        expr   = 'sph_harm(1, 1, φ, θ)'
        f      = lambda X: special.sph_harm(1, 1, X[:,0], X[:,1])
        ranges = [[0, 2*tpi], [0, tpi]]
        return symbol, expr, f, ranges

    # 133: spherical harmonics (m = 0, n = 2)
    if name == 'spherical harmonics (m = 0, n = 2)' or name == 133:
        φ, θ = symbols('φ θ')
        symbol = (φ, θ)
        expr   = 'sph_harm(0, 2, φ, θ)'
        f      = lambda X: special.sph_harm(0, 2, X[:,0], X[:,1])
        ranges = [[0, 2*tpi], [0, tpi]]
        return symbol, expr, f, ranges

    # 134: spherical harmonics (m = 1, n = 2)
    if name == 'spherical harmonics (m = 1, n = 2)' or name == 134:
        φ, θ = symbols('φ θ')
        symbol = (φ, θ)
        expr   = 'sph_harm(1, 2, φ, θ)'
        f      = lambda X: special.sph_harm(1, 2, X[:,0], X[:,1])
        ranges = [[0, 2*tpi], [0, tpi]]
        return symbol, expr, f, ranges

    # 135: spherical harmonics (m = 2, n = 2)
    if name == 'spherical harmonics (m = 2, n = 2)' or name == 135:
        φ, θ = symbols('φ θ')
        symbol = (φ, θ)
        expr   = 'sph_harm(2, 2, φ, θ)'
        f      = lambda X: special.sph_harm(2, 2, X[:,0], X[:,1])
        ranges = [[0, 2*tpi], [0, tpi]]
        return symbol, expr, f, ranges
